...
const idemStore = (orderId: string) => `delivered:order:${orderId}:store_debit` as const;
const idemCaptain = (orderId: string) => `delivered:order:${orderId}:captain_deduction` as const;
...
export async function applyDeliveredOrderLedgerTx(
  ...
  if (order.deliveryFee != null) {
    ...
    if (storeDebit.gt(0)) {
      ...
      await appendLedgerEntryInTx(tx, {
        ...
        idempotencyKey: idemStore(orderId),
        orderId: order.id,
  ...
  await appendLedgerEntryInTx(tx, {
    ...
    idempotencyKey: idemCaptain(orderId),
    orderId: order.id,
    createdByUserId: actorUserId,
    referenceType: "ORDER",
    referenceId: order.id,
