...
import { canAccessCaptainsPage, isDispatchRole } from "@/lib/rbac-roles";
...
export function useCaptains(
  ...
  const canList = isDispatchRole(role) || canAccessCaptainsPage(role);
  return useQuery({
    queryKey: queryKeys.captains.list(params),
    queryFn: () => api.captains.list(toListQuery(params)),
    enabled: (options?.enabled ?? true) && Boolean(token) && canList,
