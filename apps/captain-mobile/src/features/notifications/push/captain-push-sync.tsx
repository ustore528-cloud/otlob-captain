...
import i18n, { resolveCaptainPushLanguage } from "@/i18n/i18n";
...
if (IS_PUSH_RUNTIME_SUPPORTED) {
  Notifications.setNotificationHandler({
    handleNotification: async (notification) => {
      ...
      return {
        // Suppress default foreground alert for remote order push; we re-post via the configured order channel.
...
async function ensureAndroidChannel(): Promise<void> {
  ...
  try {
    ...
    console.info("[captain-push-sync] getNotificationChannelAsync(captain-orders-v9-strong)", {
      snapshot: ch
        ? {
            id: ch.id,
            name: ch.name,
...
export function CaptainPushSync() {
  ...
  useEffect(() => {
    if (!IS_PUSH_RUNTIME_SUPPORTED) return;
    if (!isAuthed) {
    ...
    const syncPushToken = async (reason: string) => {
      ...
      try {
        ...
        const language = await resolveCaptainPushLanguage(i18nInstance.resolvedLanguage ?? i18nInstance.language);
        const submissionKey = `${expoToken}:${language}`;
        if (submittedTokenRef.current === submissionKey) {
          // eslint-disable-next-line no-console
        ...
        const result = await captainService.registerPushTokenWithMeta(requestPayload);
        // eslint-disable-next-line no-console
        ...
        submittedTokenRef.current = submissionKey;
        // eslint-disable-next-line no-console
        console.info("[captain-push-sync] push token registered to backend successfully");
        // eslint-disable-next-line no-console
        console.info("[captain-push-sync] expo_push_token_registered", { reason, platform: Platform.OS, language });
      } catch (error) {
        // eslint-disable-next-line no-console
    ...
    const tokenSub = Notifications.addPushTokenListener((token) => {
      ...
      void (async () => {
        const language = await resolveCaptainPushLanguage(i18nInstance.resolvedLanguage ?? i18nInstance.language);
        const submissionKey = next ? `${next}:${language}` : null;
        if (!next || submittedTokenRef.current === submissionKey) return;
        // eslint-disable-next-line no-console
        console.info("[captain-push-sync] push_token_changed", { token: maskPushToken(next), language });
        void captainService
          .registerPushToken({
            token: next,
